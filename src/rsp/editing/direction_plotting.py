"""Image Plotter for nice grid images."""
...
def make_sure_vecs_point_in_same_direction(V):
    """Docstring for make_sure_vecs_point_in_same_direction.
...
def fix_sign_ambiguity(x):
    """Docstring for fix_sign_ambiguity.
...
class DirectionPlotter:
    """Docstring for DirectionPlotter."""
    ...
    def plot_direction(
        ...
        """Docstring for plot_direction.

        :param self: Description
        :param q: Description
        :param hhs_dirs: Description
        :param num_pcs: Description
        :param point_same_direction: Description
        :param scale: Description
        :param alpha_font_size: Description
        :param plot_pc_number: Description
        :param numsteps: Description
        """
        ...
        for i in tqdm(range(num_pcs)):
            ...
            lerp = self.sd.interpolate_direction(
                q,
                Q(delta_hs=hs),
                space="hspace",
                t1=-scale,
    ...
    def power_direction_overview(
        ...
        """Docstring for power_direction_overview.

        :param self: Description
        :param q: Description
        :param Uts: Description
        :param ssvals: Description
        :param global_at_idx: Description
        :param scale: Description
        :param point_same_direction: Description
        :param plot_every: Description
        :param t_idx_font_size: Description
        :param scale_fontsize: Description
        :param apply_svals: Description
        :param numsteps: Description
        :param svec_idx: Description
        ...
        for t_idx in tqdm(range(self.sd.num_inference_steps)):
            ...
            if t_idx % plot_every == 0:
                ...
                hs_local = torch.cat(
                    [
                        h[None]
                        if abs(t_idx - idx) <= window
                        else torch.zeros_like(h)[None]
                        for idx, h in enumerate(hs)
                    ]
                )
                if apply_svals:
                    hs_local = torch.cat(
                        [(s * h).unsqueeze(0) for s, h in zip(svals, hs_local)]
                    )
                lerp = self.sd.interpolate_direction(
                    q,
                    Q(delta_hs=hs_local),
                    space="hspace",
                    t1=-scale,
                    t2=scale,  # font_size = scale_fontsize,
                    numsteps=numsteps,
                    vertical=False,
                )
                imgs.append(lerp)
    ...
    def get_direction(
        ...
        hs_local = torch.cat(
            [
                h[None] if abs(t_idx - idx) <= window else torch.zeros_like(h)[None]
                for idx, h in enumerate(hs)
            ]
        )
        if apply_svals:
    ...
    def plot_timedependent_svals(self, svals, nr=5):
        """Docstring for plot_timedependent_svals.

        :param self: Description
        :param svals: Description
